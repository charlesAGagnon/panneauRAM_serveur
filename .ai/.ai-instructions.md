# 🤖 Instructions pour l'Intelligence Artificielle

> **Ce fichier est destiné aux IA (GitHub Copilot, Claude, ChatGPT, etc.) travaillant sur ce projet.**

## 📖 Lecture Obligatoire

Avant de répondre à toute question ou de générer du code, **TU DOIS LIRE** le fichier:

```
.ai/context.md
```

Ce fichier contient:

- ✅ L'architecture complète du projet PanneauRAM
- ✅ Description détaillée de tous les modules
- ✅ Conventions de code à respecter absolument
- ✅ Structure de la base de données
- ✅ Système d'alarmes et journal de bord
- ✅ Niveaux d'accès utilisateur (0-3)
- ✅ Flux de données MQTT/Socket.IO

## 🎯 Directive Principale

**TOUJOURS commencer par:**

```
1. Lire .ai/context.md
2. Comprendre le contexte de la demande
3. Respecter les conventions du projet
4. Générer le code approprié
```

## ⚠️ Règles STRICTES (de consigne.txt)

### Règle #1: AUCUN ÉMOJI dans le code

```javascript
// ❌ INTERDIT
console.log("✅ Connexion établie");

// ✅ CORRECT
console.log("Connexion etablie");
```

### Règle #2: Code SIMPLE

- Pas de sur-ingénierie
- Préférer la clarté à la sophistication
- Commenter en français les parties complexes

### Règle #3: CSS Séparé

- Jamais de style inline dans les fichiers
- Toujours utiliser les fichiers CSS existants:
  - `modern.css` - Styles principaux
  - `ui-components.css` - Composants réutilisables
  - `granulaires.css` - Page Mélangeur
  - `profils.css` - Page Profils
  - `camera.css` - Page Caméra
  - `login.css` - Page Login

## 🏗️ Architecture Clé

### Stack Technologique

- **Backend**: Node.js + Express.js
- **Base de données**: MySQL (paneauram)
- **Communication**: MQTT + Socket.IO
- **Frontend**: EJS + Vanilla JavaScript
- **Styles**: CSS3 pur (pas de framework)

### Modules MQTT Critiques

- `mqtt.js` - Panneau principal (6 mesures + commandes)
- `mqtt2.js` - Mélangeur (modes + moteurs + recettes)
- `mqtt3.js` - Power Meter (lecture seule)
- `mqtt4.js` - Aspirateur
- `mqtt5.js` - Valves (lecture seule)
- `mqtt6.js` - **Alarmes et réactions automatiques**

### Système de Journal

**IMPORTANT**: Toute commande DOIT être enregistrée

- `LOG_CMD` - Commandes utilisateur
- `LOG_ALARME` - Alarmes système

```javascript
// Exemple d'enregistrement obligatoire
journalModel.logCommand(userLogin, commandType, commandValue);
```

## 🔐 Niveaux d'Accès

| Niveau | Nom            | Accès                             |
| ------ | -------------- | --------------------------------- |
| 0      | Invité         | Lecture seule                     |
| 1      | Utilisateur    | Lecture + Commandes basiques      |
| 2      | Modérateur     | + Alarmes + Granulaires           |
| 3      | Administrateur | Accès complet + Journal + Profils |

**ATTENTION**: Toujours vérifier le niveau avant d'autoriser une action!

## 📝 Conventions de Nommage

### Variables JavaScript

```javascript
// MQTT Data
let mesures = {}; // États reçus (lecture)
let consignes = {}; // Commandes envoyées (écriture)

// Socket.IO Events
socket.emit("mqtt-publish", { topic, message, user });
socket.on("mqtt-data-panneau", callback);
```

### Routes Express

```javascript
// Toujours inclure user, niveau, typeAcces dans les queries
router.get("/page?user=admin&niveau=3&typeAcces=Administrateur");
```

### Base de données

```sql
-- Tables principales
user         -- Utilisateurs (id, nom, password, niveauAcces, typeAcces)
journal      -- Journal de bord (LogID, Type, UserLogin, ReqTime, Info)
```

## 🚨 Système d'Alarmes

### Types d'alarmes

```javascript
"ALR_GB_OVF"; // Grand Bassin - Débordement
"ALR_GB_NIV_MAX"; // Grand Bassin - Niveau Maximum
"ALR_PB_OVF"; // Petit Bassin - Débordement
"ALR_PB_NIV_MAX"; // Petit Bassin - Niveau Maximum
"ALR_CNX_BAL"; // Connexion Balance Perdue
"ALR_CNX_POW"; // Connexion Power Meter Perdue
```

### Réactions Automatiques

Quand une alarme s'active, le système réagit automatiquement:

- Débordement GB → Fermer ValveGB
- Niveau Max GB → Activer Pompe
- etc.

**IMPORTANT**: Les réactions sont dans `mqtt6.js` - Ne pas modifier sans comprendre!

## 💡 Patterns Communs

### Ajouter une nouvelle commande MQTT

```javascript
// 1. Frontend - Envoyer la commande
socket.emit("mqtt-publish", {
  topic: "RAM/device/cmd/CommandName",
  message: value,
  user: currentUser, // OBLIGATOIRE pour le journal
});

// 2. Backend (mqtt.js) - Logger la commande
if (data.user && topic.includes("/cmd/")) {
  const commandType = topic.split("/").pop();
  journalModel.logCommand(data.user, commandType, data.message);
}
```

### Créer une nouvelle page EJS

```javascript
// 1. Créer views/pages/mapage.ejs
// 2. Inclure les partials
<%- include('../partials/header', {currentPage, user, niveau, typeAcces}) %>
// 3. Créer la route dans routes/dashboard.js
// 4. Vérifier le niveau d'accès
```

## 🔄 Mise à Jour du Contexte

Ce fichier `context.md` est **automatiquement mis à jour** à chaque commit via:

- Script: `scripts/update-ai-context.js`
- Hook Git: `.git/hooks/pre-commit`

**Tu n'as JAMAIS besoin de mettre à jour manuellement la documentation!**

## 📚 Fichiers de Référence

### Documentation

- `.ai/README.md` - Vue d'ensemble du système AI
- `.ai/AI-CONTEXT-README.md` - Guide complet (EN)
- `.ai/LISEZMOI-CONTEXTE-AI.md` - Guide complet (FR)
- `.ai/CONTEXTE-AI-SYNTHESE.md` - Synthèse technique

### Configuration

- `consigne.txt` - Règles de base (pas d'émojis, code simple, CSS séparé)
- `package.json` - Dépendances
- `.gitignore` - Fichiers ignorés

## ✅ Checklist Avant de Répondre

- [ ] J'ai lu `.ai/context.md`
- [ ] Je comprends le contexte de la demande
- [ ] Je connais le niveau d'accès requis
- [ ] Je respecte les 3 règles de `consigne.txt`
- [ ] J'enregistre les commandes dans le journal
- [ ] Je vérifie la cohérence avec l'architecture existante
- [ ] Mon code est en français (commentaires, variables importantes)
- [ ] Je n'utilise AUCUN émoji dans le code

## 🎓 Exemples de Prompts Utilisateur

### Bon prompt (contexte inclus)

```
"Lis .ai/context.md. J'ai besoin d'ajouter une nouvelle alarme
pour surveiller la température. Elle doit être accessible
uniquement aux niveaux 2 et 3."
```

### Prompt incomplet (tu dois demander de lire le contexte)

```
"Ajoute une alarme de température"
→ Réponse: "Je vais d'abord lire .ai/context.md pour comprendre
le système d'alarmes existant..."
```

## 🚀 Pour Commencer

Quand l'utilisateur te demande quelque chose:

1. **Analyse le fichier `.ai/context.md`**
2. **Identifie les modules concernés** (mqtt.js, journal.js, etc.)
3. **Vérifie les permissions** (niveau 0-3)
4. **Respecte les conventions** (pas d'émojis, code simple)
5. **Propose une solution cohérente** avec l'architecture

---

**Mise à jour automatique**: ✅ Active  
**Dernière génération**: Automatique via Git hook  
**Fichier source**: Ce fichier est **manuel** et ne sera pas écrasé
